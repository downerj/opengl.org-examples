...
motion(int x, int y)
{
	...
	break;
    case LIGHT_Z:
	lightpos[Z] = (winHeight/2 - x) * 200.f/winWidth;
	glutPostRedisplay();
	break;
    case LIGHT_INTENS:
...
void
make_lightmap(GLenum light, int dim, int show)
{
...
quadratic *= 3.5f;
	...
	for(i = 0; i < dim; i++)
	{
	    dist = sqrtf((float)((dim/2.f - i) * (dim/2.f - i) +
				 (dim/2.f - j) * (dim/2.f - j)));
	    scale = 1.f/(constant + 
			 linear * dist + 
			 quadratic * dist * dist);
#if 1
	    /*if (j == 0 || i == 0 || j == dim-1 || i == dim-1) scale = 0.;*/
	    if (dist >= (dim-2)/2.f) scale = 0;
#endif
	    texture[3 * (i + dim * j) + 0] = diffuse[R] * scale + ambient[R];
	    texture[3 * (i + dim * j) + 1] = diffuse[G] * scale + ambient[G];
	    texture[3 * (i + dim * j) + 2] = diffuse[B] * scale + ambient[B];
	}

    glBindTextureEXT(GL_TEXTURE_2D, LIGHTMAP);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 
		 dim, dim, 0, GL_RGB, GL_FLOAT, texture);
    if(show)
	glDrawPixels(dim, dim, GL_RGB, GL_FLOAT, texture);
    free(texture);
    glPopAttrib();
}
...
void redraw_combomap(void)
{
    ...
    glPushMatrix();
    glLoadIdentity();
glScalef(((float)winHeight)/winWidth, 1, 1);

    glEnable(GL_TEXTURE_2D);
    glDisable(GL_LIGHTING);


    glBindTextureEXT(GL_TEXTURE_2D, SURFMAP);
...
void
help(void) {
    printf("l   - show lightmap\n");
    printf("w   - show white scene\n");
    printf("m   - unlit scene modulated by lightmap\n");
    printf("u   - unlit scene\n");
    printf("o   - lit scene using OpenGL lighting\n");
    printf("a   - lit scene using OpenGL and lightmaps\n");
    printf("p   - lit scene using lightmaps\n");
    printf("x   - surface texture + lightmaps\n");
    printf("t   - surface texture + lightmaps decrease tessellation\n");
    printf("T   - surface texture + lightmaps increase tessellation\n");
    printf("1   - surface texture + lightmaps reset tessellation\n");
}
...
reshape(int wid, int ht)
{
    ...
    a = ((float)wid)/ht;
    if (a > 1)
	glFrustum(-100.*a, 100.*a, -100., 100., 300., 501.); 
    else
	glFrustum(-100., 100., -100.*a, 100.*a, 300., 501.); 
    glMatrixMode(GL_MODELVIEW);
    CHECK_ERROR("end of reshape");
}
...
main(int argc, char *argv[])
{
	...
	surftex = (GLuint *)read_texture("../../data/brick.rgb", &texwid, &texht, &texcomps);
    ...
    glBindTextureEXT(GL_TEXTURE_2D, DEFAULT);
    ...
    make_lightmap(GL_LIGHT1, 128, FALSE);
