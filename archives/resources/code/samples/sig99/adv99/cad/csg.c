...
GLint stenSize;
enum {
    COLOR, DEPTH, STENCILVALUES, STENCILPLANES
};
int bufferInterest = COLOR;
...
drawCircle(void)
{
    ...
    for (i = 0; i < 64; i++)
	glVertex2f(cos(i * M_PI / 32), sin(i * M_PI / 32));
    glEnd();
}
...
drawPrim(int i)
{
    ...
    /*
     * Draw the motion cues for world-space transforms 
     */
    if (i == curPrim && curXform != &globalXform && !doCSG) {
	glPushMatrix();
	...
	case TRANSLATEXY:
	    glTranslatef((int) xform->translation[0], (int) xform->translation[1], xform->translation[2]);
	    ...
	    break;
	case TRANSLATEZ:
	    glTranslatef(xform->translation[0], xform->translation[1], (int) xform->translation[2]);
    ...
    /*
     * Draw the motion cues for local-space transforms 
     */
    if (i == curPrim && curXform != &globalXform && !doCSG) {
	glPushMatrix();
	glDisable(GL_LIGHTING);
	...
	case SCALEX:
	    ...
	    glRotatef(90, 0.0f, 1.0f, 0.0f);
	    glScalef(0.03f, 0.03f, 3.0f);
	    drawCylinder();
	    glColor3f(0.5f, 0.0f, 0.5f);
	    glPushMatrix();
	    glTranslatef(0.0f, 0.0f, 0.5f);
	    glScalef(1.0f, 1.0f, 0.333f);
	    drawCone();
	    glPopMatrix();
	    glPushMatrix();
	    glTranslatef(0.0f, 0.0f, -0.5f);
...
int prodAtrimmingPrims1[] =
{1};
int prodAisComplemented1[] =
{1};
int prodBtrimmingPrims1[] =
{0};
int prodBisComplemented1[] =
{0};


int prodAtrimmingPrims2[] =
{1};
int prodAisComplemented2[] =
{0};
int prodBtrimmingPrims2[] =
{0};
int prodBisComplemented2[] =
{1};


int prodAtrimmingPrims3[] =
{1};
int prodAisComplemented3[] =
{0};
int prodBtrimmingPrims3[] =
{0};
int prodBisComplemented3[] =
{0};


int prodAtrimmingPrims4[] =
{1, 2, 3};
int prodAisComplemented4[] =
{0, 1, 1};
int prodBtrimmingPrims4[] =
{1, 0, 2};
int prodBisComplemented4[] =
{0, 0, 1};
int prodCtrimmingPrims4[] =
{1, 0, 3};
int prodCisComplemented4[] =
{0, 0, 1};
int prodDtrimmingPrims4[] =
{0, 2, 3};
int prodDisComplemented4[] =
{0, 1, 1};
...
void 
copyDepthToColor(GLenum whichColorBuffer)
{
    int x,
        y;
    GLfloat max,
...
void 
copyStencilValuesToColor(GLenum whichColorBuffer)
{
    int x,
        y;
    GLint previousColorBuffer;
 ...
 */
void 
copyStencilPlanesToColor(GLenum whichColorBuffer)
{
    int x,
        y;
    GLint previousColorBuffer;
...
void 
axisamountToMat(float aa[], float mat[])
{
    float c,
          s,
          t;
...
void 
matToAxisamount(float mat[], float aa[])
{
    float c;
    float s;
...
void 
multMat(float m1[], float m2[], float r[])
{
    float t[9];
    int i;
...
void 
rotateTrackball(int dx, int dy, float rotation[4])
{
    float dist;
    float oldMat[9];
    float rotMat[9];
...
redrawNoCSG(void)
{
    ...
    /*
     * Do the global transformations, then draw 
     */
    glTranslatef(globalXform.translation[0], globalXform.translation[1], globalXform.translation[2]);
    glRotatef(globalXform.rotation[3] / M_PI * 180, globalXform.rotation[0], globalXform.rotation[1], globalXform.rotation[2]);
...
progressEnum 
renderPrimDepths(int targetPrim, int frontFace, int whichSurface)
{
    ...
    glStencilMask(sCountMask);
					 ...
					 * see comment at beginning of source 
					 */
...
progressEnum 
trimWithPrimitive(int trimPrim, int isComplemented)
{
    glDepthFunc(GL_LESS);
...
progressEnum 
markProductPixels(int product, int accumBit)
{
    int i;
...
progressEnum 
drawProduct(int product, int accumBit)
{
    struct product *p;
    p = &products[whichTree][product];
...
redrawCSG(void)
{
	...
	if (firstProduct > 0)	/*
				 * know depth is clear before 1st group 
				 */
	    saveDepth();

	accumBit = 1;		/*
				 * first available after counting bits 
				 */
	...
	if (firstProduct > 0)	/*
				 * know depth was clear before first group 
				 */
	    restoreDepth();
	else {
	    glDepthMask(GL_TRUE);
...
/*ARGSUSED*/
void 
button(int b, int state, int x, int y)
{
    ox = x;
    oy = y;
}
...
motion(int x, int y)
{
	...
	if (curXform->scale[0] < 1.f / 40.0f)
	    curXform->scale[0] = 1.f / 40.0f;
	break;

    case SCALEY:
	curXform->scale[1] += (dx + dy) / 40.0f;
	if (curXform->scale[1] < 1.f / 40.0f)
	    curXform->scale[1] = 1.f / 40.0f;
	break;

    case SCALEZ:
	curXform->scale[2] += (dx + dy) / 40.0f;
	if (curXform->scale[2] < 1.f / 40.0f)
	    curXform->scale[2] = 1.f / 40.0f;
	break;
...
mainMenuFunc(int choice)
{
	...
	if (doCSG) {
	    ...
	    glutDisplayFunc(redrawCSG);
	    glutChangeToMenuEntry(1, "Turn off CSG", 1);
	} else {
	    printf("CSG expression off\n");
	    glutDisplayFunc(redrawNoCSG);
	    glutChangeToMenuEntry(1, "Turn on CSG", 1);
	}
	glutPostRedisplay();
	break;
    case 2:
...
main(int argc, char **argv)
{
     ...
     */
    (void) glutCreateWindow("CSG using stencil   [yellow=rotate, cyan=translate, magenta=scale]");
    glutDisplayFunc(redrawNoCSG);
    ...
    glutAddMenuEntry("obj 1 MINUS obj 2 ('1')", 0);
    glutAddMenuEntry("obj 2 MINUS obj 1 ('2')", 1);
    glutAddMenuEntry("obj 1 AND obj 2 ('3')", 2);
    glutAddMenuEntry("(obj 1 AND obj 2) MINUS (obj 3 or obj 4) ('4')", 3);

    whichObjectMenu = glutCreateMenu(whichObjectFunc);
    glutAddMenuEntry("Global ('g')", 4);
    glutAddMenuEntry("Blue object #1 ('F5')", 0);
    glutAddMenuEntry("Peach object #2 ('F6')", 1);
    glutAddMenuEntry("Green object #3 ('F7')", 2);
    glutAddMenuEntry("Purple object #4 ('F8')", 3);

    morphMenu = glutCreateMenu(morphMenuFunc);
    glutAddMenuEntry("Box ('F1')", 0);
    glutAddMenuEntry("Cylinder ('F2')", 1);
    glutAddMenuEntry("Cone ('F3')", 2);
    glutAddMenuEntry("Sphere ('F4')", 3);

    moveMenu = glutCreateMenu(moveFunc);
    glutAddMenuEntry("Trackball-like rotatation ('r')", 'r');
    glutAddMenuEntry("Translate in the X-Y plane ('t')", 't');
    glutAddMenuEntry("Translate along the Z axis ('T')", 'T');
    glutAddMenuEntry("Scale in X ('x')", 'x');
    glutAddMenuEntry("Scale in Y ('y')", 'y');
    glutAddMenuEntry("Scale in Z ('z')", 'z');
    ...
    glutAddSubMenu("Movement/rotation controls", moveMenu);
